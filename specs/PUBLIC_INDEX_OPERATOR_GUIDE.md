...
When maintainers polish generated candidates, they should create separate
maintainer-curated accepted artifacts instead of editing producer output in
place. The curated package may cite the generated `specpm.yaml`,
`producer-receipt.json`, validation report, diagnostics report, and handoff
artifacts as evidence through `foreignArtifacts`, but it must not imply that
maintainer-authored summaries, capabilities, or constraints were generated by
the producer. Generated bundles remain evidence; curated accepted artifacts are
the registry source.

The curated accepted artifact lifecycle is:

1. Generated candidates are immutable producer evidence.
